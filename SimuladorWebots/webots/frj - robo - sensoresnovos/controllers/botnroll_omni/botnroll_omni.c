...
int BolaAngulos[11]={0, 35, 80, 105, 130, 160, 200, 230, 255, 280, 315};
...
//printf("{%.3lf %.3lf %.3lf} ", distancia, BolaX, BolaZ);
    ...
    switch(estado)
    {
      ...
      case 3: // orbita
              ...
              {
                if ((BOLAAng>90.0 && BOLAAng<180.0) && BolaZ>0.3) // Coordenada 0.3 a rever (deverá ser menor)
                  flag_dentro_campo=1;
                if ((BOLAAng>180.0 && BOLAAng<270.0) && BolaZ<-0.3) // Coordenada 0.3 a rever (deverá ser menor)
                  flag_dentro_campo=-1;
...
//    printf("dir(%3d) vlin(%3d) vang(%3d)\n", direccao, vel_linear, vel_angular);    
    direccao=0; vel_linear=0; vel_angular=-5;
